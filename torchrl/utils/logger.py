...
class Logger_Adaptation():
    def __init__(self, experiment_id, env_name, seed, params, task, log_dir = "./log"):

        self.logger = logging.getLogger("{}_{}_{}_{}".format(experiment_id,env_name,str(seed),task))
        ...
        work_dir = os.path.join(log_dir, experiment_id, env_name, str(seed), task)
        self.work_dir = work_dir
        if os.path.exists(work_dir):
            shutil.rmtree(work_dir)
    ...
    def add_epoch_info(self, epoch_num, eval_reward, csv_write=True):
        if csv_write:
            with open(self.csv_file_path, 'a') as f:
                self.csv_writer = csv.writer(f)
                if epoch_num == 0:
                    csv_titles = ["EPOCH", "Adaptation_Reward"]
                    self.csv_writer.writerow(csv_titles)
                csv_values = [epoch_num, eval_reward]
                self.csv_writer.writerow(csv_values)
                
        self.logger.info("EPOCH:{}".format(epoch_num))
        self.logger.info("Adaptation_Reward:{}".format(eval_reward))