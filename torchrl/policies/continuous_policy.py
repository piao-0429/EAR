...
class EmergentActionRepresentationGuassianContPolicy(networks.Net):
    
    def forward(self, lse, lte):
        x = torch.cat([lse, lte], dim=-1)
        x = super().forward(x)
        mean, log_std = x.chunk(2, dim=-1)
    ...
    def eval_act(self, lse, lte):
        with torch.no_grad():
            mean, _, _ = self.forward(lse, lte)
            
        return torch.tanh(mean.squeeze(0)).detach().cpu().numpy()

    def explore(self, lse, lte, return_log_probs = False, return_pre_tanh = False):
        
        
        mean, std, log_std = self.forward(lse, lte)
